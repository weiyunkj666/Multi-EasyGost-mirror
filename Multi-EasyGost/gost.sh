...
# ---------------------------------------------------------------------------
# 国内 GitHub 反代支持（由 cn-github-pack 注入）
#   GHCN_PROXY 是"前缀字符串"，如 https://ghfast.top/；为空串 = 直连 GitHub。
#   优先级：环境变量 GHCN_PROXY > 同目录 ghcn.sh 里选定的反代 > 直连
#   临时指定： GHCN_PROXY=https://ghfast.top/ ./gost.sh
# ---------------------------------------------------------------------------
for _ghcn in \
    "$(dirname "$0")/ghcn.sh" \
    "./ghcn.sh" \
...
# 本项目自己的 raw 地址前缀（gost.service / config.json 这些配套文件从哪拉）。
# 默认指向上游；把这份代码放进你自己的仓库后，set-repo.sh 会把它改成你自己的地址，
# 这样上游哪天删库/改名/改目录结构都不影响你。
SELF_REPO_RAW="${SELF_REPO_RAW:-https://raw.githubusercontent.com/weiyunkj666/Multi-EasyGost-mirror/main/Multi-EasyGost}"
...
function Install_ct() {
  ...
  if command -v ghcn_download >/dev/null 2>&1; then
    ...
    wget --no-check-certificate -O "gost-linux-${bit}-${ct_new_ver}.gz" "${GHCN_PROXY}https://github.com/ginuerzh/gost/releases/download/v${ct_new_ver}/gost-linux-${bit}-${ct_new_ver}.gz"
  fi
...
update_sh() {
  # 反代版改动：默认禁用"自动覆盖自己"，否则原版一更新就把国内反代补丁冲没了。
  # 想启用就设 SELF_UPDATE_URL=https://raw.githubusercontent.com/weiyunkj666/Multi-EasyGost-mirror/main/Multi-EasyGost/gost.sh
  [ -n "${SELF_UPDATE_URL:-}" ] || return 0
  ol_version=$(curl -L -s --connect-timeout 5 "${GHCN_PROXY}${SELF_UPDATE_URL}" | grep "shell_version=" | head -1 | awk -F '=|\"' '{print $3}')
  if [ -n "$ol_version" ]; then
    if [[ "$shell_version" != "$ol_version" ]]; then
      ...
      [yY][eE][sS] | [yY])
        wget -N --no-check-certificate "${GHCN_PROXY}${SELF_UPDATE_URL}"
        echo -e "更新完成"
        exit 0
        ;;
      *) ;;
