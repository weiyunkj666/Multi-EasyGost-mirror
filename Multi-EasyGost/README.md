...
> **国内反代说明（本版本已改造）**
> - 原脚本里那个"大陆镜像加速"用的是阿里云 OSS 地址，实测**已经 404 失效**，
>   本版本把它换成了自动测速的国内反代（`ghcn.sh`）。
> - 下载 gost 二进制 / gost.service / config.json 都会自动走最快反代，
>   失败还会自动依次换其他反代重试。
> - 也可以手动指定：`GHCN_PROXY=https://ghfast.top/ ./gost.sh`
> - 脚本的"自动更新"默认已关闭，避免原版把国内反代补丁覆盖掉。
>   想启用就设 `SELF_UPDATE_URL=<你自己仓库里 gost.sh 的 raw 地址>`。
## 功能
